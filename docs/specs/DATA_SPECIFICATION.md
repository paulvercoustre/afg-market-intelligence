...
| **Fallback** | Missing indicators → `NULL` in `market_context`; `score_market_quality` is `NULL` and excluded from the composite (§4.6) only if all three sub-fields (LPI, regulatory quality, political stability) are missing — otherwise averaged over whichever are available |
...
| **Cost** | Free |
| **Refresh cadence** | Monthly (with ETL) |
| **Fallback** | No tariff data → `tariff_rate_pct = NULL`, `score_tariff = NULL`, excluded from the composite (§4.6) |
...
A rate discarded this way makes `score_tariff = NULL`, excluded from the composite with the remaining weights renormalised (§4.6) — changed 2026-09-02 from a neutral-50 default. Deliberately **not** a guessed 0 either: `score_afg_foothold` already scores low (or exactly 0) for a market with no Afghan trade history — that's the correct dimension to carry that penalty, since it's a confirmed fact about that dimension specifically. `score_tariff` being `NULL` here reflects something different: we genuinely don't know what tariff would apply, not that the tariff is bad — excluding it avoids asserting either a false-favorable or false-punitive number, and avoids double-counting the same underlying "no trade evidence" fact across two dimensions the way defaulting either sub-score to a fixed guess would.
...
3. **Leave it undetermined rather than guess (UI).** When `price_competitiveness` is `NULL`, the market profile page shows "No unit data for comparison" instead of a label, plus a short note that this dimension wasn't meaningfully factored into the opportunity score for that row. Backend-side, `_score_price(None)` still returns the neutral-50 default (§4.6, "Missing data handling" — the "neutral-50 default" category, which as of 2026-09-02 is `_score_price`'s alone) — this fix doesn't change *that* mechanism, it only changes how often a genuinely unverifiable number gets treated as if it were a real one.
...
| Market quality | 13% | Average of available sub-scores (LPI, regulatory quality, political stability) — see below |
| Price competitiveness | 13% | Label-based: Substantially Below Market=100, Below Market=75, Near Market=50, Above Market=25 |
| Tariff | 12% | Log min-max vs. a fixed ceiling: `100 × (1 − log1p(rate)/log1p(35))`, clamped [0,100] — see below |
| Afghan foothold | 10% | Per-product log min-max: `100 × log1p(x) / log1p(max_x_for_product)` — see below |
| Distance | 10% | Log min-max vs. a fixed ceiling: `100 × (1 − log1p(dist_km)/log1p(20015))`, closer = higher — see below |
...
**Missing data handling — three distinct treatments, not one rule:**
- **Neutral-50 default** (`_score_price`): the dimension genuinely can't be evaluated, so it's scored as neutral rather than guessed good or bad, and the composite is still calculated as normal.
- **Excluded + weights renormalised** (`_score_market_size`, `_score_growth`, `_score_market_quality`, `_score_tariff`, `_score_distance` — see below): the underlying raw data is missing entirely (not a real zero), and there's no principled "average" or "neutral" value to assume with zero information behind it — so rather than guessing, the dimension is dropped and the remaining weights rescale to sum back to 1.0.
- **Genuine zero, not missing at all** (`_score_foothold` — see below): "no Afghan trade on record, ever" isn't an information gap, it's a confirmed fact. Scoring it `0.0` isn't a guess standing in for "unknown" — it's the accurate reflection of a known, negative fact — so this dimension was never a candidate for either of the treatments above.
...
**Market quality (average of LPI, regulatory quality, political stability — no fixed reference band needed).** Unlike market size and market growth, `_score_market_quality()` (`etl/transform.py`) doesn't need an external reference band: LPI is min-max rescaled from its known fixed 1–5 scale (`(lpi-1)/4×100`), and `regulatory_quality`/`political_stability` are already 0–100 (both fetched on the WGI "score" `.SC` variant, not the `.EST` estimate — see §4.1 above) and used as-is. The composite sub-score is the simple average of whichever of these three are available for a market.

If **all three** are missing, `score_market_quality` is `NULL`, excluded from the composite (see renormalisation below) — changed 2026-08-31 from a neutral-50 default, for the same reasoning as market size/growth: there's no principled "average" market quality to assume with zero information behind it.

**Tariff normalisation (log min-max against a fixed ceiling).** `_score_tariff()` (`etl/transform.py`) follows the same method as market size/growth/foothold: OECD (2008) Handbook, Step 5, §5.1 (log-transform for positive skew) + §5.3 (Min-Max) with the §5.4 external-reference variant (a fixed ceiling, not the observed max):

```
score = 100 × (1 − log1p(rate) / log1p(ceiling)), clamped [0,100]
```

`ceiling = TARIFF_SCORE_LOG_CEILING_PCT = 35` (`config.py`), fixed across every product and year, same stability rationale as `F`/`W` above. **Replaced 2026-09-02** — the previous formula was linear (`max(0, 100 − rate×3)`), which wasn't itself miscalibrated (its implicit ~33.3% ceiling landed close to the real p95 of 30%, checked against live data, n=916), but it still crowded almost all real tariffs into a narrow top slice of the scale, since real tariffs are positively skewed (median 3.28%, p90 20%, p99 43.2%) — most of the real variation sits in a dense cluster near zero. Log-transforming first spreads out exactly that region instead of the rare high-tariff tail: two rates a mere 2 points apart (1% and 3%) differ by ~20 points under log vs. only ~6.7 under a plain linear scale at any point on its range. A squared-ratio alternative was also checked (used for tariffs specifically by a comparable tool, [trade.gov's Market Diversification Tool](https://www.trade.gov/market-diversification-tool-indicators)) and found to do the *opposite* — it compresses the dense low-tariff region even further (76% of rows landed in a single top bin against the same real data) — so it was rejected in favor of log-transform.

**The ceiling was then re-optimised specifically for the log-transform**, not just carried over from the 30 used to compare the three candidate methods above. 32.3% of all tariff rows are an exact 0% rate (many raw goods genuinely face zero MFN tariff), which always scores exactly 100 regardless of ceiling — so differentiation power was measured only across the 620 rows with a real positive rate. Standard deviation of the resulting scores peaks broadly around ceiling=18–20 (~22.3), but that peak clips 15.6% of those rows to a flat 0 (rate ≥ ceiling). **35** trades a small amount of that peak differentiation (~21.2, ~5% less) for a much lower clip rate (5.0%) and lands the median real-tariff market exactly at neutral 50 — judged the better trade-off given how flat and broad the differentiation peak actually is.

`score_tariff` is `NULL` whenever `tariff_rate_pct` is `NULL` — whether that's because WITS has no schedule on file for that reporter/product, or because a fetched rate was discarded by the non-traded-goods filter above — excluded from the composite with the remaining weights renormalised, same treatment as market size/growth/quality. Deliberately not a guessed 0 either: `score_afg_foothold` already carries the "no Afghan trade evidence" penalty as a confirmed fact for its own dimension; `score_tariff` being unknown is a genuinely different situation (we don't know what tariff would apply, not that it's necessarily bad), so excluding it avoids asserting either a false-favorable or false-punitive number.

**Distance normalisation (log min-max against a fixed, non-arbitrary ceiling).** `_score_distance()` (`etl/transform.py`) uses the same OECD §5.1 + §5.3/§5.4 method as market size/tariff: `100 × (1 − log1p(dist_km) / log1p(ceiling))`, clamped [0,100]. `ceiling = MAX_GREAT_CIRCLE_DISTANCE_KM = 20,015` km (`config.py`) — half of Earth's circumference, the greatest possible great-circle distance between any two points on the globe. Unlike `F` (market size) or the tariff/CAGR ceilings, this constant needed no empirical derivation from this dataset's data: it's a physical fact, so no market can ever exceed it and no re-calibration is ever needed. `dist_km == 0` (Afghanistan itself, if it were ever a market) scores 100 by definition, ahead of the log formula.

**Changed 2026-09-02** — `_score_distance(None)` previously returned the neutral-50 default. `DISTANCE_FROM_KABUL_KM` is a lookup against CEPII's GeoDist dataset (Mayer & Zignago, 2011) joined via ISO-3; a market whose ISO-3 code isn't present in CEPII's data gets no entry at all, which is missing raw data, not a genuine "average distance" case. Confirmed as a real, currently-occurring gap (8 of 1042 rows, 2026-09-02): Andorra, Chile, Montenegro, "Other Asia, n.e.s.", and Palestine. `score_distance` is now `NULL` for these, excluded from the composite with the remaining weights renormalised — same treatment as market size/growth/quality/tariff.

**Renormalisation when market size, market growth, market quality, tariff, and/or distance are missing:** the composite is computed by summing each *present* dimension's `score × weight` and dividing by the sum of only the *present* dimensions' weights — so if one is missing, the remaining weights are rescaled to sum back to 1.0; if two are missing, the remaining 6 rescale together; if three are missing, the remaining 5 rescale together; if four are missing, the remaining 4 rescale together; if all five are missing, the remaining 3 rescale together. This generalises over any combination of these five being missing at once, rather than special-casing each one separately (`etl/transform.py`, `enrich_indicators_with_scores`).

**Afghan foothold (per-product log min-max — same method as market size, no external floor needed).** `_score_foothold()` (`etl/transform.py`) uses `100 × log1p(afg_export_value_usd) / log1p(max_afg_value)`, where `max_afg_value` is precomputed once per product: Afghanistan's single largest recorded export value to any market for that product (current-year or historical, whichever is larger). This means the market where Afghanistan already has its strongest presence for a given product always scores exactly 100, and every other market scales smoothly relative to it — the same per-product-ceiling property `score_market_size` has. No external floor (`F`-equivalent) is needed here, unlike market size: there's no noise/artifact ambiguity to guard against in Afghanistan's own export figures — even a tiny genuine shipment (e.g. $7) is real trade evidence (consistent with the trade-evidence rule the tariff logic in §4.1 uses), so `log1p(0) = 0` is already the correct, natural floor.

Historical fallback (no current-year trade, but a real export on record within `AFG_LAST_EXPORT_FLOOR_YEAR`): same log min-max against the same `max_afg_value`, discounted ×0.7 and capped at 90 — unchanged from before, so a stale-but-real historical presence still scores meaningfully below an active current-year one.

**Replaced 2026-09-02** — the previous formula was `min(100, log10(afg_export_value_usd + 1) × 14)`: an arbitrary multiplier with an undocumented ceiling that clamped to exactly 100 at ~$13.9M regardless of the product. Checked against live data (n=714 rows across 36 products): **22 of 36 products (61%) never had any market reach a score of 100** under that formula, purely because that product's biggest trade relationship happened to sit below the fixed $13.9M threshold — unrelated to whether it was genuinely Afghanistan's strongest market for that product. The new per-product ceiling fixes this structurally: every product gets exactly one market at 100 (its own best), never zero and never more than one tied at the flat ceiling.
